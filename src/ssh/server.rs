...
pub fn make_russh_config(cfg: &Config, keys: Vec<KeyPair>) -> Result<russh::server::Config> {
    Ok(russh::server::Config {
        ...
        inactivity_timeout: Some(std::time::Duration::from_secs(
            cfg.ssh.session_timeout_seconds,
        )),
        keys,
        ..Default::default()
    })
...
impl russh::server::Handler for HoneypotHandler {
    ...
    async fn shell_request(
        ...
        io.state = Some(SessionState::new_for_test(
            &username,
            &self.shared.cfg.ssh.hostname,
            cwd,
        ));
        if let Some(state) = io.state.as_mut() {
            ...
            match action {
                ...
                Action::AiRequest {
                    ...
                    let text = collect_ai_for_shell(&self.shared.cfg, system_prompt, user_command)
                        .await
                        .unwrap_or_else(|_| {
                            "bash: internal error: AI service unavailable\n".to_string()
                        });
                    let bytes = text.len();
                    if let Some(logger) = io.logger.as_ref() {
                        let _ = logger.log(&Event::new(
            ...
            let (response, disconnect, prompt, log_ai) = {
                ...
                match action {
                    ...
                    Action::AiRequest {
                        ...
                        let text =
                            collect_ai_for_shell(&self.shared.cfg, system_prompt, user_command)
                                .await
                                .unwrap_or_else(|_| {
                                    "bash: internal error: AI service unavailable\n".to_string()
                                });
